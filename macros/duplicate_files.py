...
IN_DIR_A = '/eos/cms/store/user/abrinke1/HiggsToAA/2DAlphabet/raw_inputs/2025_07_25/'
#IN_DIR_B = '/eos/cms/store/user/abrinke1/HiggsToAA/2DAlphabet/plots/2025_07_25/'
...
def main():
    ...
    for top_dir in [IN_DIR_A]:
        print('\n\n*** Duplicating files in %s ***' % top_dir)
        for o_dir in [top_dir+sub_dir for sub_dir in os.listdir(top_dir)]:
            if not '/VBFjj' in o_dir and not '/gg0l' in o_dir and not '/Vjj' in o_dir and not '/tt0l' in o_dir and not '/Zvv' in o_dir:
                continue  ## Only applies to hadronic categories
